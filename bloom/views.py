...
def list_course_outcomes(request):
    selected_course_id = request.GET.get('course')  # Get course ID from query parameters
    courses = Course.objects.all()
    course_outcomes = CourseOutcome.objects.all()
    
    if selected_course_id:
        selected_course = get_object_or_404(Course, pk=selected_course_id)
        course_outcomes = CourseOutcome.objects.filter(course=selected_course)  # Use `course` here
    else:
        selected_course = None  # No course selected

    return render(request, 'course_outcome_list.html', {
        'courses': courses,
...
def list_program_outcomes(request):
    ...
    if selected_course_id:
        program_outcomes = ProgramOutcome.objects.filter(course_id=selected_course_id)
    else:
        program_outcomes = ProgramOutcome.objects.all()
    
    return render(request, 'program_outcome_list.html', {
